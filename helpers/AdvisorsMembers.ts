import { member } from "./Member";
import alvaroAvatar from "../assets/advisors/alvaro-avatar.png";
import andreAvatar from "../assets/advisors/andre-avatar.png";
import justinAvatarAd from "../assets/advisors/justin-avatar.png";
import ruiAvatarAd from "../assets/advisors/rui-avatar.png";
import helderAvatarAd from "../assets/advisors/helder-avatar.png";
import marioAvatarAd from "../assets/advisors/mario-avatar.png";
import miguelAvatar from "../assets/advisors/miguel-avatar.png";
import nunoAvatar from "../assets/advisors/nuno-avatar.png";

export const advisorsMembers = [
  member(
    `Alvaro Nogueiras`,
    `Co-founder`,
    alvaroAvatar,
    `https://www.linkedin.com/in/alvarogomeznog/`
  ),
  member(
    `André Moniz`,
    `Co-founder`,
    andreAvatar,
    `https://www.linkedin.com/in/andremoniz/`
  ),
  member(
    `Hélder Vasconcelos`,
    `Co-founder`,
    helderAvatarAd,
    `https://www.linkedin.com/in/heldervasc/`
  ),
  member(
    `Justin Wu`,
    `Co-founder`,
    justinAvatarAd,
    `https://twitter.com/theWUJustin`
  ),
  member(
    `Mário Alves`,
    `Co-founder`,
    marioAvatarAd,
    `https://www.linkedin.com/in/marioribeiroalves/`
  ),
  member(
    `Miguel Leite`,
    `Co-founder`,
    miguelAvatar,
    `https://www.linkedin.com/in/miguelleite`
  ),
  member(
    `Nuno Correia`,
    `Co-founder`,
    nunoAvatar,
    `https://www.linkedin.com/in/nuno-m-correia`
  ),
  member(
    `Rui Teixeira`,
    `Co-founder`,
    ruiAvatarAd,
    `https://www.linkedin.com/in/rui-dos-santos-teixeira`
  ),
];
